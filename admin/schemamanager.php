...
if ($isCli) {
	function usage(string $script): never
		{
			fwrite(STDERR, <<<TEXT
		Usage:
		php {$script} --dbuser <value> --dbpassword <value>

		Options:
		--dbuser <value>	(required) database user name
		--dbpassword <value>	(required) database user password
    ...
    $options = getopt("", ["dbuser:", "dbpassword:", "help"]);

	if (isset($options['help'])) {
        usage($argv[0]);
    }

	if (!isset($options['dbuser']) || trim($options['dbuser']) === '') {
        fwrite(STDERR, "Error: Missing required option --dbuser <value>.\n\n");
        usage($argv[0]);
    }

	if (!isset($options['dbpassword']) || trim($options['dbpassword']) === '') {
        fwrite(STDERR, "Error: Missing required option --dbpassword <value>.\n\n");
        usage($argv[0]);
    }
...
$schemaManager = new SchemaManager($isCli);
...
if ($isCli) {
	if($curentVersion == $releaseVersion) echo 'Schema is Current - nothing to do';
	if($curentVersion){
		foreach($schemaPatchArr as $schemaOption){
			if($schemaOption > $curentVersion){
				$schemaCode = $schemaOption;
				echo 'Attempting to update to ' . $schemaCode;
				break;
			}
		}
	...
	$schemaManager->setDBPassword($password);
