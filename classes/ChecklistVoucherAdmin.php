...
class ChecklistVoucherAdmin extends Manager {
	...
	protected function encodeStr($inStr){
		...
		if($inStr && $charSetSource){
			if($charSetOut == 'UTF-8'){
				$retStr = mb_convert_encoding($inStr, 'UTF-8', mb_detect_encoding($inStr, 'UTF-8,ISO-8859-1'));
			}
			elseif($charSetOut == 'ISO-8859-1'){
				$retStr = mb_convert_encoding($inStr, 'ISO-8859-1', mb_detect_encoding($inStr, 'UTF-8,ISO-8859-1'));
			}
			else{
				$retStr = mb_convert_encoding($inStr, $charSetOut, mb_detect_encoding($inStr, 'UTF-8,ISO-8859-1'));
			}
		}
		return $retStr;
	}
