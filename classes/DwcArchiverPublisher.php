...
class DwcArchiverPublisher extends DwcArchiverCore{
	...
	public function writeRssFile(){
		...
		if($sourcePath == $deprecatedPath || !file_exists($deprecatedPath)){
			...
			$redirectDoc->loadXML('<redirect><newLocation>' . GeneralUtilities::getDomain() . $GLOBALS['CLIENT_ROOT'] . '/content/dwca/rss.xml</newLocation></redirect>');
	...
	public function getCollectionList($catID){
		$retArr = array();
		$serverName = GeneralUtilities::getDomain();
