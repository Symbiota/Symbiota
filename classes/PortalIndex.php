...
class PortalIndex extends OmCollections{
	...
	public function getSelfDetails(){
		...
		$retArr['urlRoot'] = GeneralUtilities::getDomain().$GLOBALS['CLIENT_ROOT'];
	...
	public function initiateHandshake($remotePath){
		...
		if($remotePath){
			...
			if($remoteArr){
				if($remoteArr['guid']){
					$handShakeUrl = GeneralUtilities::getDomain().$GLOBALS['CLIENT_ROOT'].'/api/v2/installation/'.$remoteArr['guid'].'/touch?endpoint='.$remoteArr['urlRoot'];
