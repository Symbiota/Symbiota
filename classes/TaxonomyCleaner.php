					...
					if($matchArr = $taxonHarvester->getCloseMatch($sciname)){
						...
						foreach($matchArr as $tid => $scinameMatch){
							...
							$echoStr = '<i>'.implode(' ',$snTokens).'</i> =&gt; <span class="hideOnLoad">wait for page to finish loading...</span><span class="displayOnLoad" style="display:none">'.
								'<a href="#" onclick="return remappTaxon(\'' . urlencode($r->sciname) . '\',' . htmlspecialchars($tid, HTML_SPECIAL_CHARS_FLAGS) . ',\'' . htmlspecialchars($idQual, HTML_SPECIAL_CHARS_FLAGS) . '\',' . htmlspecialchars($itemCnt, HTML_SPECIAL_CHARS_FLAGS) . ')" style="color:blue"> remap to this taxon</a>' . 
