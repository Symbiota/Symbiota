...
class VoucherVisionBatchHandler {
    ...
    public function runVoucherVision($imageUrls, $prompt, $engines, $ocrOnly, $llmModel) {
        ...
        // Create annotations for successful results
        $this->createAnnotations($results, $prompt);
        
        return $results;
    }
    
    /**
     * Create annotations for successful VoucherVision results
     *
     * @param array $results Results from VoucherVision API calls
     * @param string $prompt The prompt used for transcription
     * @return void
     */
    private function createAnnotations($results, $prompt) {
        global $PARAMS_ARR;
        
        // Get current username from global variables
        $currentUser = $PARAMS_ARR['un'] ?? '';
        
        // Only create annotations if we have a logged-in user
        ...
        foreach ($results as $occid => $result) {
            if ($result['success'] && isset($result['formatted_json'])) {
                try {
                    ...
                    $fieldMapping = $this->getFieldMapping($prompt);
                    if (!$fieldMapping) {
                        continue; // Skip if no mapping available
                    }
                    
                    // Map VoucherVision fields to Symbiota fields
                    $mappedData = [];
                    foreach ($result['formatted_json'] as $vvField => $value) {
                        if (isset($fieldMapping[$vvField]) && !empty($value)) {
                            $symbiotaField = $fieldMapping[$vvField];
                            $mappedData[$symbiotaField] = $value;
                        }
                    }
    ...
    /**
     * Get the web root path for API URL construction
     *
     * @return string The web root path (e.g., '/portal')
     */
    private function getWebRoot() {
        ...
        if (isset($_SERVER['SCRIPT_NAME'])) {
            ...
            if (strpos($scriptName, '/') !== false) {
                ...
                // Find the portal/application part in the path
                for ($i = 0; $i < count($pathParts); $i++) {
                    if ($pathParts[$i] === 'portal' || $pathParts[$i] === basename($GLOBALS['SERVER_ROOT'] ?? '')) {
                        $webRoot = '/' . implode('/', array_slice($pathParts, 0, $i + 1));
                        break;
                    }
                }
                if (empty($webRoot)) {
                    // Fallback: use first path component if it looks like an app directory
                    if (count($pathParts) > 0 && strlen($pathParts[0]) > 0) {
                        $webRoot = '/' . $pathParts[0];
                    }
                }
            }
    ...
    /**
     * Make a request to create an annotation via the API
     *
     * @param int $occid Occurrence ID
     * @param array $data Mapped field data
     * @param string $currentUser Current username
     * @return bool Success status
     */
    private function makeAnnotationRequest($occid, $data, $currentUser) {
        global $SECURITY_KEY;
        
        // Construct the annotation API URL
        ...
        $apiUrl = $baseUrl . $webRoot . '/api/v2/occurrence/annotation/insert';
        
        // Prepare the request parameters
        $params = [
            'apiToken' => $SECURITY_KEY,
            'occid' => $occid,
            'externalSource' => 'vouchervision',
            'externalEditor' => $currentUser,
            'appliedStatus' => 0
        ];
        ...
        // Initialize cURL for the annotation request
        $curl = curl_init();
        ...
        // Check for errors
        if ($curlError) {
            error_log("Annotation API cURL error for occid {$occid}: " . $curlError);
            return false;
        }
        
        if ($httpCode !== 200 && $httpCode !== 201) {
            error_log("Annotation API HTTP error for occid {$occid}: HTTP {$httpCode}");
            return false;
        }
        
        $decodedResponse = json_decode($response, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            error_log("Annotation API JSON decode error for occid {$occid}: " . json_last_error_msg());
            return false;
        }
        
        // Log successful annotation creation
        if (isset($decodedResponse['status']) && $decodedResponse['status'] === 'SUCCESS') {
            error_log("Successfully created annotation for occid {$occid} via VoucherVision");
        }
