...
class WordCloud{
	...
	public function batchBuildWordClouds($csMode = 0){
		$processingArr = array();
		$sql = 'SELECT DISTINCT c.collid FROM omoccurrences o INNER JOIN media m ON o.occid = m.occid INNER JOIN specprocessorrawlabels r ON m.mediaID = r.imgid ';
	...
	public function buildWordCloud($collid, $csMode = 0){
		...
		$sql = 'SELECT DISTINCT r.rawstr FROM omoccurrences o INNER JOIN media m ON o.occid = m.occid INNER JOIN specprocessorrawlabels r ON m.mediaID = r.imgid ';
		if($csMode) $sql .= 'INNER JOIN omcrowdsourcequeue q ON o.occid = q.occid ';
		$sql .= 'WHERE o.processingstatus = "unprocessed" AND o.locality IS NULL ';
