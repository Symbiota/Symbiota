...
function get_taxon_names() {
   ...
   while($row = $taxons->fetch_assoc()) {
      ...
      if($row['Synonym'] != null) array_push($taxon_map[$row['tid']]['synonyms'], $row['Synonym']);
   }
...
if(isset($GLOBALS['SYMB_UID'])) {
   foreach(get_taxon_names() as $tid => $taxon) {
      ...
      if(!$statements) {
         foreach($taxon['synonyms'] as $synonym) {
            $statements = getNorthAmericanFloraStatements($synonym);
            if($statements) break;
         }
      } else {
